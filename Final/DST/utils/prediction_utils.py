...
from utils.input_example import (
    STATUS_ACTIVE,
    STATUS_DONTCARE,
    STR_DONTCARE,
)
...
def get_predicted_dialog(dialog: dict, all_predictions: dict, schemas: object, state_tracker: str) -> dict:
    ...
    for turn_idx, turn in enumerate(dialog["turns"]):
        if "frames" not in list(turn.keys()):
            # if turn["speaker"] == "SYSTEM" and state_tracker == 'nemotracker':
            #     turn["frames"] = []
            #     for serv_i, serv in enumerate(turn["service"]):
            #         tmp_frame = {}
            #         if serv not in sys_slots_agg:
            #             sys_slots_agg[serv] = OrderedDict()
            #         tmp_frame["actions"] = [] 
            ...
            if turn["speaker"] == "USER":
                ...
                dialog["turns"][turn_idx]["frames"] = []
                serv_list = []
                if type(turn["service"]) == list:
                    serv_list += turn["service"]
                else:
                    serv_list.append(turn["service"])
                for serv_i, serv in enumerate(serv_list):
                    tmp_frame = {}
                    tmp_frame["service"] = serv
                    # print("serv: ", serv)
                    predictions = all_predictions[(dialog_id, turn_id, serv)]
                    # print("All predictions:")
                    # for k in all_predictions.keys():
                    #     print("{}-{}".format(k,all_predictions[k]))
                    slot_values = all_slot_values[serv]
                    service_schema = schemas.get_service_schema(serv)
                    # Remove the slot spans and state if present.
                    # dialog["turns"][turn_idx]["frame"].pop("slots", None)
                    # dialog["turns"][turn_idx]["frame"].pop("state", None)
                    ...
                    noncat_out_dict = set_noncat_slot(
                        ...
                        sys_slots_agg=sys_slots_agg.get(serv, None),
                    )
                    ...
                    tmp_frame["state"] = state 
                    dialog["turns"][turn_idx]["frames"].append(tmp_frame)           
...
def write_predictions_to_file(
    ...
    for idx, prediction in enumerate(predictions):
        ...
        #     all_predictions[(dialog_id, turn_id, service_name)] = prediction
            ...
            #     print("{}:{}".format(k,v))
    # if split == "dev":
    #     print("\ndev in write prediction")
    #     print(list(all_predictions.keys())[0], " ", all_predictions[list(all_predictions.keys())[0]])
    #     print()
    # elif split == "test":
    #     print("\ntest in write prediction")
    #     print(list(all_predictions.keys())[0], " ", all_predictions[list(all_predictions.keys())[0]])
    #     print()
