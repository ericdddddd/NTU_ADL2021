...
FILE_RANGES = {
    "sgd_single_domain": {"train": range(1, 92), "dev": range(1, 15), "test": range(1, 2)},
    "seen_domain": {"train": range(1, 2), "dev": range(1, 2), "test": range(1, 17)},
    "unseen_domain": {"train": range(1, 2), "dev": range(1, 2), "test": range(1, 6)},
    # "sgd_single_domain": {"train": range(1, 61), "dev": range(1, 15), "test": range(1, 17)},
    # "sgd_single_domain": {"train": range(1, 60), "dev": range(1, 15), "test": range(1, 17)},
    # "sgd_single_domain": {"train": range(95, 96), "dev": range(1, 2), "test": range(1, 2)}, 
    "sgd_multi_domain": {"train": range(44, 128), "dev": range(8, 21), "test": range(12, 35)},
    "sgd_all": {"train": range(1, 96), "dev": range(1, 21), "test": range(1, 17)},
...
class SGDDataProcessor(object):
    ...
    def _generate_dialog_examples(self, dataset_split: str, schemas: object, subsample: bool):
        ...
        for dialog_idx, dialog in enumerate(dialogs):
            ...
            #     if dialog_id.isupper():
            #         r = re.compile("([a-zA-Z]+)([0-9]+)")
            #         m = r.match(dialog_id)
            #         dialog_id_1 = m.group(1)
            #         dialog_id_2 = m.group(2)
            #         tmp_id = dialogid_dict.setdefault(dialog_id_1, str(len(dialogid_dict)))
            #         dialogs[dialog_idx]['dialogue_id'] = tmp_id + '_' + dialog_id_2

            #         file = open("dialog_id.pkl", "wb")
            #         pickle.dump(dialogid_dict, file)
            #         file.close()
    ...
    def _create_examples_from_dialog(
        ...
        """
        dialog_id = dialog["dialogue_id"]
        dialog_service = dialog['services']
        prev_states = {}
        examples = []
        for turn_idx, turn in enumerate(dialog["turns"]):
            ...
            if turn["speaker"] == "USER":
                ...
                if dataset_split == 'test':
                    # user_frames = {turn["service"]:[]}
                    user_frames = {serv:[] for serv in turn["service"]}
                    # user_frames = {serv:[] for serv in dialog_service}
                else:
                ...
                if turn_idx > 0:
                    ...
                    if dataset_split == 'test':
                        # system_frames = {turn["service"]:[]}
                        system_frames = {serv:[] for serv in turn["service"]}
                        # system_frames = {serv:[] for serv in dialog_service}
                    elif system_turn['frames'] == []:
                        # system_frames = {f["service"]: [] for f in turn["frames"]}
                        system_frames = {}
                    else:
    ...
    def _create_examples_from_turn(
        ...
        # if dataset_split == "test":
        #     for service in list(user_frames.keys()):
        ...
        #         except:
        #             print(dataset_split," ", dialog_id," ",turn_id_)
        #         base_example.example_id = f"{turn_id}"
        ...
        #         # print("example id: ", np.array(base_example.example_id_num, dtype = np.int))
        #         # print("service_id:", np.array(base_example.service_id))
        #         # print("utterance_id: ", np.array(base_example.utterance_ids))
        #         # print("utterance_segment: ", np.array(base_example.utterance_segment))
        #         # print("utterance_mask: ", np.array(base_example.utterance_mask, dtype=np.long))
        #         # print()
        ...
        for service, user_frame in user_frames.items():
            ...
            # print("state_update before: {}".format(state_update))
            states[service] = state
            # print("states after assign service: {}".format(states))
            dataset_split, dialog_id, turn_id_ = turn_id.split('-')
            dialog_id_1, dialog_id_2 = dialog_id.split('_')
            ...
            for model_task in range(self.schema_config["NUM_TASKS"]):
                ...
                if model_task == 2:
                    ...
                    for slot_id, slot in enumerate(schemas.get_service_schema(service).categorical_slots):
                        ...
                        # if task_example.categorical_slot_status == 0 or dataset_split == 'test':
                ...
                if model_task == 4:  # noncat slot status
                    ...
                    for slot_id, slot in enumerate(schemas.get_service_schema(service).non_categorical_slots):
                        ...
                        # if task_example.noncategorical_slot_status == 0 or dataset_split == 'test':
                            off_slots.append(task_example)
                        else:
                            on_slots.append(task_example)
                            examples.append(task_example)
        ...
        # print("\n----------------------------Datae preprocess test----------------------------")
        # print("example:")
        # for ie in examples:
        #     # print(ie)
        #     print("{:15}|{}".format("schema_config", ie.schema_config))
        #     print("{:15}|{}".format("service_schema",ie.service_schema ))
        #     print("{:15}|{}".format("service_id", ie.service_id))
        #     print("{:15}|{}".format("example_id", ie.example_id))
        #     print("{:15}|{}".format("example_id_num", ie.example_id_num))
        #     print("{:15}|{}".format("user_utterance", ie.user_utterance))
        #     print("{:15}|{}".format("system_utterance", ie.system_utterance))
        #     print("{:15}|{}".format("utterance_ids", ie.utterance_ids))
        #     print("{:15}|{}".format("utterance_segment", ie.utterance_segment))
        #     print("{:15}|{}".format("utterance_mask", ie.utterance_mask))
        #     print("{:15}|{}".format("start_char_idx", ie.start_char_idx))
        #     print("{:15}|{}".format("end_char_idx", ie.end_char_idx))
        #     print("{:15}|{}".format("categorical_slot_id", ie.categorical_slot_id))
        #     print("{:15}|{}".format("noncategorical_slot_id", ie.noncategorical_slot_id))
        #     print("{:15}|{}".format("categorical_slot_status", ie.categorical_slot_status))
        #     print("{:15}|{}".format("noncategorical_slot_status", ie.noncategorical_slot_status))
        #     print("{:15}|{}".format("task_mask", ie.task_mask))
        #     print("{:15}|{}".format("noncategorical_slot_value_start", ie.noncategorical_slot_value_start))
        #     print("{:15}|{}".format("noncategorical_slot_value_end", ie.noncategorical_slot_value_end))
        #     print("{:15}|{}".format("categorical_slot_value_id", ie.categorical_slot_value_id))
        #     print("{:15}|{}".format("categorical_slot_value_status", ie.categorical_slot_value_status))
        #     print("{:15}|{}".format("requested_slot_id", ie.requested_slot_id))
        #     print("{:15}|{}".format("requested_slot_status", ie.requested_slot_status))
        #     print("{:15}|{}".format("intent_id", ie.intent_id))
        #     print("{:15}|{}".format("intent_status", ie.intent_status))
        #     print()
        # print("\n")
        # print("state after:\n", state, "\n\n")
        # print("slot_carryover_values:\n", slot_carryover_values, "\n")
            ...
            for slot_span in char_slot_spans:
                if slot_span["slot"] == slot:
                    try:
                        ...
                        # print('wrong value:', utterance[slot_span["start"] : slot_span["exclusive_end"]])
                        for k in list(slot_span.keys()):
                            print(k,":",slot_span[k])
                        print()
