...
from utils.data_processor import SGDDataProcessor
from utils.schema import Schema
from utils.dataset import SGDDataset
from utils.evaluate import evaluate, get_in_domain_services
from utils.prediction_utils import write_predictions_to_file
from utils.sgd_loss import SGDDialogueStateLoss
from utils.nlp_model import NLPModel
...
class SGDQAModel(NLPModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        # print(self._cfg.dataset.task_name)
    ...
    def test_step(self, batch: List[torch.Tensor], batch_idx: int, dataloader_idx: int = 0) -> dict:
        ...
        Args:
            ...
            dataloader_idx: dataloader index
        """
        tensors = self.test_step_helper(batch=batch)
        return {f'tensors': tensors}
    ...
    def test_step_helper(self, batch: List[torch.Tensor]):
        """
        Helper called at every test step to aggregate and postprocess outputs on each GPU
        Args:
            batch: input batch at step
        Returns:
            tensors: collection of aggregated output tensors across all GPU workers
    ...
    def multi_test_epoch_end(self, outputs: List[dict], dataloader_idx: int = 0):
        ...
        self.multi_test_epoch_end_helper(outputs=outputs, split=split, dataloader=dataloader)
    ...
    def multi_eval_epoch_end_helper(
        ...
        try:
            ...
            print("Cannot find the saving folder!")
            prediction_dir = "./"
