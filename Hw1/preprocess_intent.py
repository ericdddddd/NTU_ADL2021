...
def build_vocab(
    ...
    common_words = {w for w, _ in words.most_common(vocab_size)} #default = 10,000

    logging.info(f"vocab_size : {str(vocab_size)}")
    ...
    with open(glove_path,encoding="utf-8") as fp:
        ...
        for i, line in tqdm(enumerate(fp)):
            ...
            word = cols[0]
            vector = [float(v) for v in cols[1:]] # 300-Dim
    ...
    assert all(len(v) == glove_dim for v in glove.values()) # confirm all vectors have same length
    assert len(glove) <= vocab_size # confirm glove vocs smaller than vocab_size
...
def main(args):
    ...
    for split in ["train", "eval"]:
        ...
        intents.update({instance["intent"] for instance in dataset}) # 取得類別
        words.update(
            [token for instance in dataset for token in instance["text"].split()] #取得單字
        )
